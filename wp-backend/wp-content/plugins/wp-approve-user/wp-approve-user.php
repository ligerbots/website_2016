...
if ( false ) {
	...
	add_action( 'update_option_users_can_register', 'wpau_whitelist_users', 10, 2 );
	return;
}
...
class Obenland_Wp_Approve_User extends Obenland_Wp_Plugins_v301 {
	...
	public function user_row_actions( $actions, $user_object ) {
		if ( ( get_current_user_id() != $user_object->ID ) AND current_user_can( 'edit_user', $user_object->ID ) ) {
			...
			if ( get_user_meta( $user_object->ID, 'wp-approve-user', true ) ) {
				...
				$actions['wpau-unapprove'] = sprintf( '<a class="submitunapprove" href="%1$s">%2$s</a>', esc_url( $url ), __( 'Unapprove', 'wp-approve-user' ) );
				...
				$actions['wpau-approve'] = sprintf( '<a class="submitapprove" href="%1$s">%2$s</a>', esc_url( $url ), __( 'Approve', 'wp-approve-user' ) );
			}
		}
	...
	public function admin_action_wpau_update() {
		...
		switch( $_REQUEST['update'] ) {
			case 'wpau-approved':
				$message = _n( 'User approved.', '%d users approved.', $count, 'wp-approve-user' );
				break;

			case 'wpau-unapproved':
				$message = _n( 'User unapproved.', '%d users unapproved.', $count, 'wp-approve-user' );
				break;
			default:
	...
	protected function check_user() {
		...
		if ( ! current_user_can( 'promote_users' ) ) {
			wp_die( __( 'You can&#8217;t unapprove users.', 'wp-approve-user' ), '', array(
				'back_link' => true,
			) );
		}
